((): void => {
	...
	let expandBtnRef: HTMLButtonElement | null = null;

	function createQuestButton(): void {
		if (!window.location.pathname.includes("/quest-home")) {
			const existingButton = document.getElementById(
				"discord-quest-helper-btn",
			);
			if (existingButton) {
    ...
    `;
		...
		button.addEventListener("click", (): void => {
			if (typeof chrome === "undefined" || !chrome.runtime) {
				console.error(
					"Chrome runtime not available. Make sure the extension is loaded.",
				);
			...
			chrome.runtime.sendMessage(
				...
				(response: { success: boolean; error?: string } | undefined): void => {
	...
	function createExpandedPanel(): void {
    ...
    `;
		...
		title.style.cssText =
			"margin: 0 0 12px 0; font-size: 16px; font-weight: bold;";
	...
	function toggleExpanded(): void {
		...
		if (expandBtnRef) {
			expandBtnRef.style.transform = isExpanded
				? "rotate(180deg)"
				: "rotate(0deg)";
		}
		if (isExpanded) {
